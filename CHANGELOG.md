...
### Changed
- **删除豆袋微动效（v2 设计稿）**：二次确认改为整张卡片 3D 翻面（0.55s，正反面 `backface-visibility: hidden`，非激活面 `inert` 不可聚焦）；确认删除后卡片先塌缩淡出退场（约 0.38s），**退场动画结束后才写入数据**并弹 toast；toast 撤销时卡片以塌缩态插回原位再展开返场。toast 本体新增底部浮入浮出与 2px 倒计时细线（6 秒线性缩短，与撤销窗口同步，`showToast` 新增 `countdown` 选项）。豆袋列表由 grid 改为单列 flex（440px 容器下本就单列，视觉不变），由每张卡片的 wrapper 自带下边距，塌缩时下方卡片自然顺势补位。`prefers-reduced-motion: reduce` 下全部动效退化为直接切换（数据立即写入）

### Added
- **删除豆袋**：首页每张豆袋卡片右上角新增常驻淡显的 `×` 按钮。点击后卡片**原位**变为二次确认状态（「删除这袋豆子？」+ 删除/留着，不弹窗、不跳布局）；确认删除立即生效并持久化，底部 toast 提供「撤销」（与「丢弃这一杯」同一模式：撤销窗口仅存内存，刷新即不可撤销），撤销后豆袋恢复原顺序位置。卡片根元素由 `<button>` 改为 `<div role="button" tabindex="0">`（按钮不能嵌套按钮），Enter/Space 键盘导航保持可用
